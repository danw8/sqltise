...
impl Character {
	pub fn new(value: &str, byte_length: usize) -> Result<Self, SqlTypeError> {
		if value.to_lowercase().trim() == "null" {
			return Err(SqlTypeError { message: "Character cannot be null".to_string() });
		}
		Ok(
			Character {
				byte_length,
...
impl SqlType for Character {
	 fn validate(&self) -> Result<(), ErrorMessage> {
		 if self.value.bytes().len() > self.byte_length {
			 return Err(ErrorMessage("Value is longer than expected length".to_string()));
		 }
		 if !self.value.is_ascii() {
			 return Err(ErrorMessage("Value can only contains ascii characters".to_string()));
		 }
		 Ok(())
	 }
...
impl NullCharacter {
	pub fn new(value: &str, byte_length: usize) -> Result<Self, SqlTypeError> {
		...
		Ok( NullCharacter { value: Some(character)})
	}
}
