...
impl SqlType for DateTime2 {
	fn validate(&self) -> Result<(), ErrorMessage> {
		if self.value < NaiveDateTime::new(NaiveDate::from_ymd(0001, 01, 01), NaiveTime::from_hms(0, 0, 0)) {
...
impl NullDateTime2 {
	pub fn new(value: &str) -> Result<Self, SqlTypeError> {
		...
		Ok( NullDateTime2 { value: Some(date_time_2) })
	}
}
