	...
	pub fn new(value: &str) -> Result<Self, SqlTypeError> {
		...
		for format in &DATETIME_OFFSET_FORMATS {
			...
			if parsed.is_ok() {
				...
				let date_time_2 = DateTimeOffset { value };
...
impl SqlType for DateTimeOffset {
	fn validate(&self) -> Result<(), ErrorMessage> {
		let year_one = DateTime::<FixedOffset>::from_utc(
			NaiveDateTime::new(
		...
		let year_nine_nine_nine = DateTime::<FixedOffset>::from_utc(
			NaiveDateTime::new(
				NaiveDate::from_ymd(9999, 12, 31),
				NaiveTime::from_hms_milli(23, 59, 59, 9999999),
			),
			FixedOffset::west(0),
		);
...
impl NullDateTimeOffset {
	pub fn new(value: &str) -> Result<Self, SqlTypeError> {
		...
		Ok(NullDateTimeOffset {
			value: Some(date_time_offset),
		})
	}
}
...
impl SqlType for NullDateTimeOffset {
	fn validate(&self) -> Result<(), ErrorMessage> {
		match &self.value {
			Some(date_time_offset) => date_time_offset.validate(),
			None => Ok(()),
		}
	}
