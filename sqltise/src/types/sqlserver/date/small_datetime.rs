	...
	pub fn new(value: &str) -> Result<Self, SqlTypeError> {
		...
		for format in &DATETIME_FORMATS {
			...
			if parsed.is_ok() {
				...
				let small_date_time = SmallDateTime {
					value
				};
...
impl SqlType for SmallDateTime {
	fn validate(&self) -> Result<(), ErrorMessage> {
		if self.value < NaiveDateTime::new(NaiveDate::from_ymd(1900, 01, 01), NaiveTime::from_hms(0, 0, 0)) {
			return Err(ErrorMessage("Value is less than '1900-01-01 00:00:00'".to_string()));
		}
		if self.value >= NaiveDateTime::new(NaiveDate::from_ymd(2079, 06, 06), NaiveTime::from_hms(23, 59, 59)) {
			return Err(ErrorMessage("Value is greater than '2079-06-06 23:59:59'".to_string()));
		}
		Ok(())
	}
...
impl NullSmallDateTime {
	pub fn new(value: &str) -> Result<Self, SqlTypeError> {
		...
		Ok( NullSmallDateTime { value: Some(small_date_time) })
	}
}
...
impl SqlType for NullSmallDateTime {
	fn validate(&self) -> Result<(), ErrorMessage> {
		match &self.value {
			Some(small_date_time) =>  {
				small_date_time.validate()
			},
			None => {
				Ok(())
			}
