...
pub fn get_headers(csv_data: &str) -> Result<Vec<Header>, SqltiseError>  {
    ...
    let header_values = reader.headers()
...
"#;
        ...
        assert_eq!(result[0], Header { index: 0, value: "One".into() });
        assert_eq!(result[1], Header { index: 1, value: "Two".into() });
        assert_eq!(result[2], Header { index: 2, value: "Three".into() });
    }
